...
defineProps(mergeProps(PrimitiveProps, {
  ...
  index: {
    type: [String, Number],
    required: true,
    desc: "the index position to locate the record in the client stats array"
  }
}))
...
<template>
 <Primitive ref="primitiveElement" :as-child="asChild" :as="as">
    <slot :label="rootContext.getClient().stats[index].label">
        <template v-if="index?.toString() && index !== null">
            {{ rootContext.getClient().stats[index].label }}
        </template>
    </slot>
 </Primitive>
</template>